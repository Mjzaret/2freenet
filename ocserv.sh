...
_EOF_
	...
	sed -i "s/tcp-port = 443/tcp-port = ${port}/g" "${confdir}/ocserv.conf"
    	...
    	sed -i "s/default-domain = example.com/default-domain = 2freenet.com/g" "${confdir}/ocserv.conf"
	...
	sed -i -e '/#nas-identifier/s/my-server-name/2freenet.com/g' "/etc/radcli/radiusclient.conf"
