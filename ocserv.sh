...
_EOF_
	...
	sed -i 's@server-cert = /etc/pki/ocserv/public/server.crt@server-cert = /root/fullchain.pem@g' "${confdir}/ocserv.conf"
	sed -i 's@server-key = /etc/pki/ocserv/private/server.key@server-key = /root/privkey.pem@g' "${confdir}/ocserv.conf"
