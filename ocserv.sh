...
function ConfigEnvironmentVariable {
    # Variable settings
    # Single IP maximum number of connections, the default is 1
    maxsameclients=1
...
_EOF_
	...
	sed -i 's@auth = "pam"@#auth = "pam"\nauth = "radius [config=/etc/radcli/radiusclient.conf,groupconfig=true]"\nacct = "radius [config=/etc/radcli/radiusclient.conf,groupconfig=true]"@g' "${confdir}/ocserv.conf"
	...
	sed -i "s/rate-limit-ms = 100/rate-limit-ms = ${ratelimitms}/g" "${confdir}/ocserv.conf"
	sed -i "s/auth-timeout = 240/auth-timeout = ${authtimeout}/g" "${confdir}/ocserv.conf"
	sed -i "s/#idle-timeout = 1200/idle-timeout = ${idletimeout}/g" "${confdir}/ocserv.conf"
	sed -i "s/#mobile-idle-timeout = 2400/mobile-idle-timeout = ${mobileidletimeout}/g" "${confdir}/ocserv.conf"
	sed -i "s/min-reauth-time = 300/min-reauth-time = ${minreauthtime}/g" "${confdir}/ocserv.conf"
	sed -i "s/max-ban-score = 80/max-ban-score = ${maxbanscore}/g" "${confdir}/ocserv.conf"
	sed -i "s/ban-reset-time = 1200/ban-reset-time = ${banresettime}/g" "${confdir}/ocserv.conf"
	...
	sed -i 's@tls-priorities = "NORMAL:%SERVER_PRECEDENCE"@#tls-priorities = "NORMAL:%SERVER_PRECEDENCE"\ntls-priorities="NORMAL:%SERVER_PRECEDENCE:%COMPAT:-RSA:-VERS-SSL3.0:-ARCFOUR-128:-VERS-TLS1.0:-VERS-TLS1.1"@g' "${confdir}/ocserv.conf"
	sed -i 's@server-cert = /etc/pki/ocserv/public/server.crt@server-cert = /root/2freenet.info/fullchain.pem@g' "${confdir}/ocserv.conf"
	sed -i 's@server-key = /etc/pki/ocserv/private/server.key@server-key = /root/2freenet.info/privkey.pem@g' "${confdir}/ocserv.conf"
	sed -i "s/tcp-port = 443/tcp-port = ${port}/g" "${confdir}/ocserv.conf"
    sed -i "s/udp-port = 443/#udp-port = ${port}/g" "${confdir}/ocserv.conf"
    sed -i "s/default-domain = example.com/default-domain = 2freenet.info/g" "${confdir}/ocserv.conf"
    ...
    sed -i "s/cookie-timeout = 300/#cookie-timeout = 300/g" "${confdir}/ocserv.conf"
    sed -i 's/user-profile = profile.xml/#user-profile = profile.xml/g' "${confdir}/ocserv.conf"
	sed -i 's@no-route = 192.168.5.0/255.255.255.0@#no-route = 192.168.5.0/255.255.255.0@g' "${confdir}/ocserv.conf"
	sed -i -e '/#nas-identifier/s/my-server-name/2freenet.info/g' "/etc/radcli/radiusclient.conf"
	sed -i -e 's/#nas-identifier/nas-identifier/g' "/etc/radcli/radiusclient.conf"
	sed -i -e '/authserver/s/localhost/ibs.2freenet.com/g' "/etc/radcli/radiusclient.conf"
	sed -i -e '/acctserver/s/localhost/ibs.2freenet.com/g' "/etc/radcli/radiusclient.conf"
	sed -i -e '$a\ibs.2freenet.com\t\t\t\t123456789' "/etc/radcli/servers"
